...
/**
 * Error Boundary para la ruta base /join
 */
export default function JoinErrorBoundary({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error('[Join Error Boundary] Error capturado en /join:', error)
  }, [error])
    ...
    <div className="flex min-h-screen flex-col items-center justify-between p-4 sm:p-6 bg-background text-foreground">
      ...
      <div className="w-full max-w-md my-auto rounded-[32px] border border-destructive/30 bg-card p-6 sm:p-8 shadow-2xl flex flex-col items-center text-center animate-fade-in">
        ...
        <p className="mt-2 text-sm text-muted-foreground max-w-xs">
          Ha ocurrido un problema inesperado al cargar los datos de la invitación. Por favor, reintenta o accede desde el inicio.
        </p>
